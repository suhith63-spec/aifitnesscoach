...
# Define relevant landmarks indices if MediaPipe is available
if MEDIAPIPE_AVAILABLE:
    # These are already initialized in the try block above
...
def count_repetition_push_up(
    detector, img, landmark_list, stage, counter, exercise_instance
):
...
def count_repetition_bicep_curl(
    detector, img, landmark_list, stage_right, stage_left, counter, exercise_instance
):
    right_arm_angle = detector.find_angle(img, 12, 14, 16)
    left_arm_angle = detector.find_angle(img, 11, 13, 15)
...
def count_repetition_shoulder_press(
    detector, img, landmark_list, stage, counter, exercise_instance
):
    right_arm_angle = detector.find_angle(img, 12, 14, 16)
    left_arm_angle = detector.find_angle(img, 11, 13, 15)
...
class Exercise:
    ...
    def extract_features(self, landmarks):
        ...
        if len(landmarks) == len(relevant_landmarks_indices) * 3:
            # Angles
            features.append(
                calculate_angle(landmarks[0:3], landmarks[6:9], landmarks[12:15])
            )  # LEFT_SHOULDER, LEFT_ELBOW, LEFT_WRIST
            features.append(
                calculate_angle(landmarks[3:6], landmarks[9:12], landmarks[15:18])
            )  # RIGHT_SHOULDER, RIGHT_ELBOW, RIGHT_WRIST
            features.append(
                calculate_angle(landmarks[18:21], landmarks[24:27], landmarks[30:33])
            )  # LEFT_HIP, LEFT_KNEE, LEFT_ANKLE
            features.append(
                calculate_angle(landmarks[21:24], landmarks[27:30], landmarks[33:36])
            )  # RIGHT_HIP, RIGHT_KNEE, RIGHT_ANKLE
            features.append(
                calculate_angle(landmarks[0:3], landmarks[18:21], landmarks[24:27])
            )  # LEFT_SHOULDER, LEFT_HIP, LEFT_KNEE
            features.append(
                calculate_angle(landmarks[3:6], landmarks[21:24], landmarks[27:30])
            )  # RIGHT_SHOULDER, RIGHT_HIP, RIGHT_KNEE

            # New angles
            features.append(
                calculate_angle(landmarks[18:21], landmarks[0:3], landmarks[6:9])
            )  # LEFT_HIP, LEFT_SHOULDER, LEFT_ELBOW
            features.append(
                calculate_angle(landmarks[21:24], landmarks[3:6], landmarks[9:12])
            ...
            distances = [
                ...
                calculate_distance(
                    landmarks[18:21], landmarks[21:24]
                ),  # LEFT_HIP, RIGHT_HIP
                calculate_distance(
                    landmarks[18:21], landmarks[24:27]
                ),  # LEFT_HIP, LEFT_KNEE
                calculate_distance(
                    landmarks[21:24], landmarks[27:30]
                ),  # RIGHT_HIP, RIGHT_KNEE
                calculate_distance(
                    landmarks[0:3], landmarks[18:21]
                ),  # LEFT_SHOULDER, LEFT_HIP
                calculate_distance(
                    landmarks[3:6], landmarks[21:24]
                ),  # RIGHT_SHOULDER, RIGHT_HIP
                calculate_distance(
                    landmarks[6:9], landmarks[24:27]
                ),  # LEFT_ELBOW, LEFT_KNEE
                calculate_distance(
                    landmarks[9:12], landmarks[27:30]
                ),  # RIGHT_ELBOW, RIGHT_KNEE
                calculate_distance(
                    landmarks[12:15], landmarks[0:3]
                ),  # LEFT_WRIST, LEFT_SHOULDER
                calculate_distance(
                    landmarks[15:18], landmarks[3:6]
                ),  # RIGHT_WRIST, RIGHT_SHOULDER
                calculate_distance(
                    landmarks[12:15], landmarks[18:21]
                ),  # LEFT_WRIST, LEFT_HIP
                calculate_distance(
                    landmarks[15:18], landmarks[21:24]
            ...
            y_distances = [
                calculate_y_distance(
                    landmarks[6:9], landmarks[0:3]
                ),  # LEFT_ELBOW, LEFT_SHOULDER
                calculate_y_distance(
                    landmarks[9:12], landmarks[3:6]
            ...
            print(
                f"Insufficient landmarks: expected {len(relevant_landmarks_indices)}, got {len(landmarks) // 3}"
            )
    ...
    def auto_classify_and_count(self):
        """
        Auto classify exercises and count their repetitions from a video stream.
        """
        stframe = st.empty()
        ...
        while True:
            ...
            if len(landmarks_window) == window_size:
                landmarks_window_np = (
                    np.array(landmarks_window).flatten().reshape(1, -1)
                )
                scaled_landmarks_window = self.scaler.transform(landmarks_window_np)
                scaled_landmarks_window = scaled_landmarks_window.reshape(
                    1, window_size, 22
                )
            ...
            detector.find_person(
                frame, draw=True
            )  # Ensuring landmarks are drawn on the frame
            landmark_list = detector.find_landmarks(
                frame, draw=True
            ...
            if len(landmark_list) > 0:
                ...
                if current_prediction == "push-up":
                    stages["push_up"], counters["push_up"] = count_repetition_push_up(
                        detector,
                        frame,
                        landmark_list,
                        stages["push_up"],
                    ...
                    stages["squat"], counters["squat"] = count_repetition_squat(
                        detector,
                        frame,
                        landmark_list,
                        stages["squat"],
                    ...
                    (
                        stages["right_bicep_curl"],
                        stages["left_bicep_curl"],
                        counters["bicep_curl"],
                    ...
                    stages["shoulder_press"], counters["shoulder_press"] = (
                        count_repetition_shoulder_press(
                            detector,
                            frame,
                            landmark_list,
                            stages["shoulder_press"],
                            counters["shoulder_press"],
                            self,
                        )
                    )
            ...
            text_size, _ = cv2.getTextSize(
                f"Exercise: {short_name}", cv2.FONT_HERSHEY_SIMPLEX, 1.5, 2
            )
            draw_styled_text(
                frame, f"Exercise: {short_name}", ((width - 290) // 2 + 100, 20)
            )

            for idx, (exercise, count) in enumerate(counters.items()):
                short_name = exercise_name_map.get(exercise, exercise)
                draw_styled_text(
                    frame, f"{short_name}: {count}", (10, (idx + 1) * vertical_spacing)
                )
    ...
    def push_up(self, cap, is_video=False, counter=0, stage=None):
        self.exercise_method(
            cap, is_video, count_repetition_push_up, counter=counter, stage=stage
        )

    # Define squat method
    def squat(self, cap, is_video=False, counter=0, stage=None):
        self.exercise_method(
            cap, is_video, count_repetition_squat, counter=counter, stage=stage
        )

    # Define bicep curl method
    def bicept_curl(
        self, cap, is_video=False, counter=0, stage_right=None, stage_left=None
    ):
        self.exercise_method(
            cap,
            is_video,
    ...
    def shoulder_press(self, cap, is_video=False, counter=0, stage=None):
        self.exercise_method(
            cap, is_video, count_repetition_shoulder_press, counter=counter, stage=stage
        )
            ...
            while cap.isOpened():
                ...
                while frame_count < target_frame:
                    ...
                    if frame_count == target_frame:
                        ...
                        if len(landmark_list) != 0:
                            if multi_stage:
                                stage_right, stage_left, counter = (
                                    count_repetition_function(
                                        detector,
                                        img,
                                        landmark_list,
                            ...
                            if self.are_hands_joined(
                                landmark_list, stop=False, is_video=is_video
                            ):
                                return
